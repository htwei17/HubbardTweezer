...
> 2. uppercase `U`, `V`, `T`: the target values are fixed to the values given at the first iteration of equalization, with all physical trap parameters being the initial guess. The target values cannot be set by external input except that the $U/t$ ratio can be set by `U_over_t` parameter in the "input in `[Equalization_Result]`" section.
