try:
    from ortools.graph.python import linear_sum_assignment as ortools_linear_sum_assignment
except ImportError:
    ortools_linear_sum_assignment = None
...
def nearest_match(site: np.ndarray, wf: np.ndarray) -> np.ndarray:
    ...
    if ortools_linear_sum_assignment is None:
        row_ind, col_ind = scipy_linear_sum_assignment(dist_mat)
        order[row_ind] = col_ind
        optimal_cost = dist_mat[row_ind, col_ind].sum()
    else:
        ...
        assignment = ortools_linear_sum_assignment.SimpleLinearSumAssignment()
        ...
        optimal_cost = assignment.optimal_cost()

    if optimal_cost > threshold:  # If any WF is way too far from its site
        print(
            f"WARNING: total distance = {optimal_cost}.",
