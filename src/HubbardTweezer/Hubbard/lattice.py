...
def quadrant_ring(n, theta, radius, idx, sectors, nodes):
    ...
    for i in range(n):
        coord = (
            radius
            * sec
            * np.array([np.sin(theta * (2 * i + 1)), np.cos(theta * (2 * i + 1))])
        )
...
def subdivide(triangles: np.ndarray, nodes: np.ndarray) -> np.ndarray:
    ...
    for color, A, B, C in triangles:
        if color == 0:
            ...
            result = np.append(result, [(0, C, P, B), (1, P, C, A)], axis=0)
        else:
...
def sqr_lattice(size: np.ndarray):
    ...
    for i in range(len(edge[0])):
        for j in range(len(edge[1])):
            ...
            node_idx_pair = np.append(
                node_idx_pair, [[edge_idx[0][i], edge_idx[1][j]]], axis=0
            )
            if i > 0:  # Row link
                links = np.append(links, [[node_idx - size[1], node_idx]], axis=0)
            if j > 0:  # Column linke
...
def tri_lattice(size: np.ndarray, major_centered: bool = True, symmetry: bool = True):
    ...
    for j in range(len(edge[1])):
        ...
        if major_centered:
            ...
            major_row = j % 2 == 0
        edge_i, edge_idx_i = (
            (edge[0], edge_idx[0]) if major_row else (edge[-1], edge_idx[-1])
        )
        for i in range(len(edge_i)):
            nodes = np.append(nodes, [[edge_i[i], edge[1][j]]], axis=0)
            node_idx_pair = np.append(
                node_idx_pair, [[edge_idx_i[i], edge_idx[1][j]]], axis=0
            )
            if i > 0:  # Row link
                links = np.append(links, [[node_idx - 1, node_idx]], axis=0)
            if j > 0:  # Column link
                if symmetry:
                    if major_row:  # Major row
                        if i > 0:  # Leftward link
                            links = np.append(
                                links, [[node_idx - size[0], node_idx]], axis=0
                            )
                        if i < len(edge[0]) - 1:  # Righttward link
                            links = np.append(
                                links, [[node_idx - size[0] + 1, node_idx]], axis=0
                            )
                    else:  # Minor row
                        links = np.append(
                            links, [[node_idx - size[0], node_idx]], axis=0
...
def hc_lattice(size, lesshole: bool = True):
    ...
    if not lesshole:
        major_row = ~major_row
    hole = np.logical_and(major_row, node_idx_pair[:, 0] % 3 == 2)
    hole = np.logical_or(
        hole, np.logical_and(np.logical_not(major_row), node_idx_pair[:, 0] % 3 == 0)
    )
    hole_idx = np.nonzero(hole)[0]
    nodes = nodes[~hole, :]
    links = squeeze_idx(links, hole_idx)
...
def kagome_lattice(size):
    ...
    hole = np.logical_and(node_idx_pair[:, 1] % 4 == 1, node_idx_pair[:, 0] % 2 == 1)
    hole = np.logical_or(
        hole, np.logical_and(node_idx_pair[:, 1] % 4 == 3, node_idx_pair[:, 0] % 2 == 0)
    )
    hole_idx = np.nonzero(hole)[0]
    nodes = nodes[~hole, :]
    links = squeeze_idx(links, hole_idx)
...
def squeeze_idx(links: np.ndarray, hole_idx: np.ndarray) -> np.ndarray:
    ...
    links = links[~np.any(np.isin(links, hole_idx), axis=1), :]  # Remove links to holes
    ...
    loolup_table = np.concatenate((idx_wo_hole[:, None], shift_no[:, None]), axis=1)
    ...
    if symmetry:
        ...
        for i in range(graph.shape[0]):
            if all(graph[i, :] <= 0):
                pp = i  # [1 1] sector
                mp = np.nonzero(
                    np.prod(np.array([[-1, 1]]) * graph == graph[i, :], axis=1)
                )[0][
                    0
                ]  # [-1 1] sector
                pm = np.nonzero(
                ...
                inv_coords = np.append(inv_coords, np.array([graph[i, :] == 0]), axis=0)
    else:
