...
% Tweezer Params
Mx = 2 * wx; % full length across -x to +x
My = 2 * wy;
Mz = 4 * max(zRx, zRy);
Nx = 15; % full number of gridpoints, the actual dx = Mx / (Nx-1)
Ny = 15;
Nz = 31;
% Mx = 0.3 * wx;
% My = 0.3 * wx;
...
TTx = spdiags(ones(numel(xx), 1) * pi^2/3, 0, Nx * Ny * Nz, Nx * Ny * Nz); % diagonal term
