...
class DVRDynamicsIO:
    def __init__(self, t=None, gs=None, wavefunc=False, trap=(None, None)) -> None:
        self.t = t
        self.rho_gs = gs
    ...
    def write_file(self, fn: str):
        with h5py.File(fn, "a") as f:
            ...
            if self.wavefunc:
                ...
                # if __debug__:
                #     print('trap OK')
                append_to_table(f, "psi", self.psi.astype(np.complex), dtype=np.complex)
                # if __debug__:
                #     print('wavefunc OK')
...
def append_to_table(f: h5py.File, dset: str, t, dtype=float):
    ...
    if dset in f.keys():
        ...
        f.create_dataset(
            dset, data=t, dtype=dtype, chunks=True, maxshape=(None, t.shape[1])
        )
