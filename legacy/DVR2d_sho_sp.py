...
def Vmat(n, dx):
    ...
    V = potential(X[0], X[1]).transpose(
        1, 0)  # 3 index tensor V(x, y, z)
...
def Hop(T, V, n, psi0):
    ...
    psi += np.einsum('ij,jk->ik', T[0], psi0)  # T_xx' delta_yy' delta_zz'
    psi += np.einsum('jk,ik->ij', T[1], psi0)  # delta_xx' T_yy' delta_zz'
