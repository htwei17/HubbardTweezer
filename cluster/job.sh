...
__conda_setup=\"\$('/opt/apps/software/Miniforge3/24.1.2-0/bin/conda' 'shell.bash' 'hook' 2> /dev/null)\"
if [ \$? -eq 0 ]; then
    eval \"\$__conda_setup\"
else
    if [ -f \"/opt/apps/software/Miniforge3/24.1.2-0/etc/profile.d/conda.sh\" ]; then
        . \"/opt/apps/software/Miniforge3/24.1.2-0/etc/profile.d/conda.sh\"
    else
        export PATH=\"/opt/apps/software/Miniforge3/24.1.2-0/bin:\$PATH\"
    fi
fi
unset __conda_setup

if [ -f \"/opt/apps/software/Miniforge3/24.1.2-0/etc/profile.d/mamba.sh\" ]; then
    . \"/opt/apps/software/Miniforge3/24.1.2-0/etc/profile.d/mamba.sh\"
fi
# <<< conda initialize <<<
