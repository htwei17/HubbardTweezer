...
class Unitaries(EuclideanEmbeddedSubmanifold):
    ...
    def inner(self, point, tangent_vector_a, tangent_vector_b):
        return np.tensordot(tangent_vector_a.conj(),
                            tangent_vector_b,
    ...
    def ehess2rhess(self, point, euclidean_gradient, euclidean_hvp,
                    tangent_vector):
        Xt = multihconj(point)
    ...
    def log(self, point_a, point_b):
        U = multiprod(multihconj(point_a), point_b)
        if self._k == 1:
    ...
    def randvec(self, point):
        ...
        nrmU = np.sqrt(
            np.tensordot(tangent_vector.conj(),
                         tangent_vector,
